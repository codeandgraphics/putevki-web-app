...
class BaseController extends Controller
{
    ...
    public function initialize()
    {
        if (array_key_exists(SearchQuery::LAST_QUERIES_KEY, $_COOKIE)) {
            $this->lastQueries = json_decode(
                $_COOKIE[SearchQuery::LAST_QUERIES_KEY]
            );
        }

        $this->params = Params::getInstance();

        $this->cities = Cities::find([
            'active = 1',
            'order' => 'main DESC, name'
        ]);
        $this->branches = Branches::findPublic(['active = 1']);

        $popularBuilder = $this->modelsManager
            ->createBuilder()
            ->columns(['tourvisor.*'])
        ...
        $this->departure = Departures::findFirst(
            'id=' . $this->params->search->from
        );
