...
class CitiesController extends ControllerBase
{
    ...
    public function cityAction($cityId)
    {
        ...
        foreach ($countries as $country) {
            if ($country->tourvisorId > 0) {
                $formCountries[$country->tourvisorId] =
                    $country->tourvisor->name;
            }
        }
        ...
        $countriesSelect = new Select('popularCountries[]', $formCountries, [
            'multiple' => true
        ]);
        ...
        if ($this->request->isPost()) {
            ...
            if ($city->save()) {
                ...
                return $this->response->redirect(
                    $this->backendUrl->get('cities/city/' . $city->id)
                );
            }
    ...
    public function branchAddAction($cityId)
    {
        ...
        if ($this->request->isPost()) {
            ...
            $branch->additionalEmails = $this->request->getPost(
                'additionalEmails'
            );
            ...
            $branch->metaDescription = $this->request->getPost(
                'metaDescription'
            );
        ...
        if ($this->request->isPost()) {
            ...
            if ($form->isValid()) {
                ...
                if (!$branch->manager) {
                    ...
                    $manager->email = $branch->email;
                    $manager->company = $branch->name;
                    $manager->password = $this->security->hash(
