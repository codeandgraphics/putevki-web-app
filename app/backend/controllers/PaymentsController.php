            ...
            if ($paymentSum) {
                ...
                if ($payment->save()) {
                    $this->flashSession->success(
                        'Создан платеж на ' .
                            Text::humanize('price', $paymentSum) .
                            ' руб.'
                    );
                } else {
    ...
    public function paymentAction()
    {
        ...
        if ($this->request->has('confirmPayment')) {
            ...
            if (is_array($data)) {
                ...
                $payment->save();
                $this->flashSession->success(
                    'Авторизация платежа успешно подтверждена'
                );
            } else {
                $this->flashSession->error(
                    'Авторизация платежа не подтверждена. Ошибка: ' . $data
                );
            }
        }
        ...
        if ($this->request->has('update')) {
            ...
            if ($orderData) {
                $payment->status = \Phalcon\Text::lower(
                ...
                $this->flashSession->error(
                    'Ошибка при обновлении данных платежа'
                );
            }
        }
