...
class RequestsController extends ControllerBase
{
    ...
    public function addAction()
    {
        ...
        $form->add(
            new Select('departureId', $departures, ['using' => ['id', 'name']])
        );

        $tourOperators = Tourvisor\Operators::find();
        $form->add(
            new Select('tourOperatorId', $tourOperators, [
                'using' => ['id', 'name']
            ])
        );
        ...
        $form->add(
            new Select('requestStatusId', $requestStatuses, [
                'using' => ['id', 'name']
            ])
        );
            ...
            if ($form->isValid()) {
                ...
                if ($request->save()) {
                    ...
                    return $this->response->redirect(
                        $this->backendUrl->get('requests/edit/' . $request->id)
                    );
                } else {
        ...
        if ($this->user->role === Users::ROLE_ADMIN) {
            ...
            $form->add(
                new Select('branchId', $branches, ['using' => ['id', 'name']])
            );
        }
    ...
    public function editAction($requestId)
    {
        ...
        if ($this->request->isPost()) {
            ...
            if ($touristIds) {
                $phql =
                    'DELETE FROM Backend\Models\RequestTourists WHERE requestId = :requestId:';
    ...
    public function ajaxHotelsAction()
    {
        if ($this->request->isGet()) {
            ...
            $query = "SELECT * FROM \Models\Tourvisor\Hotels
						...
						LIMIT 20";
            $hotels = $this->modelsManager->executeQuery($query, [
                'search' => '%' . $search . '%'
            ]);
