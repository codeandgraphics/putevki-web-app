...
class EmailController extends ControllerBase
{
    public function testAction($type)
    {
        ...
        switch ($type) {
            ...
            case 'password':
                ...
                break;
            case 'payment':
                $this->sendManagerPaymentNotification(
    ...
    public function sendManagerPaymentNotification($payment, $isTest = false)
    {
        ...
        $body = $this->generate('managerPaymentNotification', [
            'payment' => $payment
        ]);
    ...
    public function sendPassword($email, $password, $isTest = false)
    {
        ...
        if ($isTest) {
            ...
            $mailgun->send(
                $email,
                'Регистрация в панели управления Путёвки.ру',
                $body
            );
        }
    }
    ...
    public function sendAdminNotification(int $requestId, $isTest = false)
    {
        ...
        $tour->orderName =
            $request->subjectName . ' ' . $request->subjectSurname;
        ...
        if ($isTest) {
            ...
            $mailgun = new Mailgun();
            $mailgun->send(
                $this->config->defaults->requestsEmail,
                'Новая заявка на тур',
                $body
            );
        }
        ...
        if ($request->branchId) {
            ...
            if ($isTest) {
                ...
                $mailgun->send(
                    ...
                    'Новая заявка на тур',
                    $body,
                    $request->branch->additionalEmails
                );
            }
        }
    }
    ...
    public function sendRequest($type, int $requestId, $isTest = false)
    {
        ...
        $tour->agreementLink = $this->url->get(
            'tour/agreement/' . $request->id
        );
    ...
    public function sendFindTour($data, $isTest = false)
    {
        ...
        if ($isTest) {
            ...
            $mailgun->send(
                $this->config->defaults->requestsEmail,
                'Заявка на подбор тура на Путёвки.ру',
                $body
            );
        }
    }
    ...
    public function sendTourHelp($phone, $queries, $isTest = false)
    {
        ...
        if ($isTest) {
            ...
            $mailgun->send(
                $this->config->defaults->requestsEmail,
                'Заявка на звонок на Путёвки.ру',
                $body
            );
        }
    }
