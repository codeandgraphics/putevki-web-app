...
import { isScrolledIntoView } from '../utils/helpers.es6';
...
export default class Search {
  ...
  filterPrice(from, to) {
    this.$.items.find('.item:not(.template)').each((i, item) => {
      let shownCount = 0;

      $(item).find('.variant').addClass('hiddenPrice').each((k, variant) => {
        const price = parseInt($(variant).attr('data-price'), 10);

        if (price >= from && price <= to) {
          $(variant).removeClass('hiddenPrice');
          shownCount += 1;
        }
      });

      if (shownCount === 0) {
        $(item).addClass('hiddenPrice');
      } else {
